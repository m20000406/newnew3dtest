...
void graphic::outputDebugInfs() {
	using namespace std;
	wstring str(L"position:(");
	str += to_wstring(g_pos.x) + wstring(L",") + to_wstring(g_pos.y) + wstring(L",")
		+ to_wstring(g_pos.z) + wstring(L")\n");
	str += wstring(L"right degree:");
	str += to_wstring(atan2(g_eye.x, g_eye.z)*180/PI) + wstring(L"\n");
	str += wstring(L"up degree:");
	str += to_wstring(atan2(-g_eye.y, g_eye.z)*180/PI) + wstring(L"\n");
	d2d::outputDebugInfs(str);
}
#endif